    ...
    <form id="update-movie-form" enctype="multipart/form-data" 
                ...
                @foreach ($generes as $item)
                    <div class="form-check">
                        <input type="checkbox"
                               ...
                               {{ in_array($item->id, old('category', $movie->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            @if ($movie->poster)
                <div style="margin-top:10px;">
                    <img src="{{ asset('storage/'.$movie->poster) }}" alt="Current Poster" style="max-width:200px; border:1px solid #ccc; padding:5px;">
                </div>
            @endif
        ...
        <div class="form-group">
            ...
            <input type="url" name="trailer" id="trailer" class="form-input" 
                   value="{{ old('trailer', $movie->trailer_url) }}" required>
        </div>

        <!-- Featured & Trending -->
        <div class="form-group">
            <label>
                <input type="checkbox" name="is_featured" {{ old('is_featured', $movie->is_featured) ? 'checked' : '' }}> Featured
            </label>
            <label style="margin-left:20px;">
                <input type="checkbox" name="is_trending" {{ old('is_trending', $movie->is_trending) ? 'checked' : '' }}> Trending
            </label>
        </div>
